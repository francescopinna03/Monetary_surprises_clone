...
if [ $# -ge 2 ]; then
    export ANNOUNCEMENT_ROTATION_DRAWS="$2"
else
    unset ANNOUNCEMENT_ROTATION_DRAWS || true
fi
...
"$matlab_bin" -batch "cd('$repo_dir'); Announcement_risk_rotation" 2>&1 \
    | tee "$repo_dir/risk_rotation_run.log"
